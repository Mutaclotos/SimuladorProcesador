...
public class Nucleo extends Thread
{
	...
	private Lock cache;
	private Lock direcP0 = new ReentrantLock();
	private Lock direcP1 = new ReentrantLock();
	private Lock busDatos = new ReentrantLock();
	private Lock cacheRemota1 = new ReentrantLock();
	private Lock cacheRemota2 = new ReentrantLock();
	private Lock cacheRemota3 = new ReentrantLock();
		...
		//while(!verificarCola())	
		{
			...
			//synchronized(procesador.colaContextos)
			{
				...
				synchronized (procesador.colaContextos) //Si la cola de contextos no est� bloqueada, bloquearla
				{
					...
					{
						System.out.println("Contexto  " + procesador.colaContextos.get(0).getEtiqueta()
					...
					System.out.println("Nucleo " + nombre + " del Procesador " + procesador.nombre
							+ " ejecutando hilillo " + etiquetaContexto);
				}
				...
				//synchronized(procesador.colaContextos)
				{
					if (instruccion[0] == 63) //Si se llego a la instruccion FIN, se saca el contexto del hilillo de la cola de contextos
					{
						synchronized (procesador.colaContextos)
						{
							...
							{
								procesador.colaContextos.get(getIndiceCola(etiquetaContexto))
										.setTiempoEjecucion(fin - inicio);
								copiarAContexto(procesador.colaContextos.get(getIndiceCola(etiquetaContexto)),
										fin - inicio); //Se copian los valores de registro y pc al contexto relevante
								Contexto contextoRemovido = procesador.colaContextos
										.remove(getIndiceCola(etiquetaContexto)); //Se elimina el contexto del hilillo de la cola de contextos
							...
							{
								System.out.println(
										"----Contexto no encontrado en la cola del procesador " + procesador.nombre);
							}

							System.out.println("Ejecucion de hilillo " + etiquetaContexto + " finalizada por el nucleo "
									+ nombre + " del Procesador " + procesador.nombre);
						}
					} else //Si se acaba el quantum para este hilillo, se realiza un cambio de contexto
					{
						synchronized (procesador.colaContextos)
						{
							if (getIndiceCola(etiquetaContexto) != -1)
							{
								...
								copiarAContexto(procesador.colaContextos.get(getIndiceCola(etiquetaContexto)),
										fin - inicio); //Se copian los valores de registro y pc al contexto relevante
								System.out.println("Cambio de contexto del nucleo " + nombre + " del Procesador "
										+ procesador.nombre);
							} else
							{
								System.out.println(
	...
	public void ejecutarOperacion(int[] ins)
	{
		...
		{
		case 8: // daddi
			System.out.println("Nucleo " + nombre + " de Procesador " + procesador.nombre + " Ejecutando DADDI "
					+ ins[1] + ", " + ins[2] + ", " + ins[3] + " :");
			this.registro[ins[2]] = this.registro[ins[1]] + ins[3];
			this.pc += 4;
			break;
		case 32: // dadd
			System.out.println("Nucleo " + nombre + " de Procesador " + procesador.nombre + " Ejecutando DADD " + ins[1]
					+ ", " + ins[2] + ", " + ins[3] + " :");
			this.registro[ins[3]] = this.registro[ins[1]] + this.registro[ins[2]];
			this.pc += 4;
			break;
		case 34: // dsub
			System.out.println("Nucleo " + nombre + " de Procesador " + procesador.nombre + " Ejecutando DSUB " + ins[1]
					+ ", " + ins[2] + ", " + ins[3] + " :");
			this.registro[ins[3]] = this.registro[ins[1]] - this.registro[ins[2]];
			this.pc += 4;
			break;
		case 12: // dmul
			System.out.println("Nucleo " + nombre + " de Procesador " + procesador.nombre + " Ejecutando DMUL " + ins[1]
					+ ", " + ins[2] + ", " + ins[3] + " :");
			this.registro[ins[3]] = this.registro[ins[1]] * this.registro[ins[2]];
			this.pc += 4;
			break;
		case 14: // ddiv
			System.out.println("Nucleo " + nombre + " de Procesador " + procesador.nombre + " Ejecutando DDIV " + ins[1]
					+ ", " + ins[2] + ", " + ins[3] + " :");
			this.registro[ins[3]] = this.registro[ins[1]] / this.registro[ins[2]];
			this.pc += 4;
			break;
		case 4: // beqz
			System.out.println("Nucleo " + nombre + " de Procesador " + procesador.nombre + " Ejecutando BEQZ " + ins[1]
					+ ", " + ins[3] + " :");
			if (this.registro[ins[1]] == 0)
				this.pc += ins[3] * 4;
			break;
		case 5: // bnez
			System.out.println("Nucleo " + nombre + " de Procesador " + procesador.nombre + " Ejecutando BNEZ " + ins[1]
					+ ", " + ins[3] + " :");
			if (this.registro[ins[1]] != 0)
				this.pc += ins[3] * 4;
			break;
		case 3: // jal
			System.out.println(
					"Nucleo " + nombre + " de Procesador " + procesador.nombre + " Ejecutando JAL " + ins[3] + " :");
			this.registro[31] = this.pc;
			this.pc += ins[3];
			break;
		case 2: // jr
			System.out.println(
					"Nucleo " + nombre + " de Procesador " + procesador.nombre + " Ejecutando JR " + ins[1] + " :");
	...
	public synchronized int[] getInstruccion()
	{
		...
		{//Se copia una palabra de la cache de instrucciones a la variable a retornar
			System.arraycopy(procesador.cacheInstrucciones[posicionCacheX], posicionCacheY, instruccion, 0,
					instruccion.length);
		}
	...
	private synchronized void copiarAcacheInstrucciones()
	{
		...
		synchronized (procesador.memInstrucciones)
		{
			...
			for (int i = 0; i < 4; i++)//Se copia el bloque entero de la memoria de instrucciones a una matriz temporal
			{
				...
				System.arraycopy(procesador.memInstrucciones, convertirPC() + indice, tempArray[i], 0,
						tempArray[i].length);
				indice += 4;
			}
		...
		synchronized (procesador.cacheInstrucciones)
		{
			...
			{
				System.arraycopy(tempArray[i], 0, procesador.cacheInstrucciones[i], posicionCacheY,
						procesador.cacheInstrucciones[i].length / 4);
			}

			//imprimirArreglo(procesador.cacheInstrucciones[posicionCacheX], procesador.cacheInstrucciones[posicionCacheX].length);
			procesador.cacheInstrucciones[4][convertirDireccionAPosicionCache(pc) * 4] = convertirDireccionANumBloque(
					pc); //Se actualiza la etiqueta del bloque
						...
						if (estado == 2)
						{
							...
							if ((bloqueVictMem < 16 && this.nombreP == 0) || (bloqueVictMem > 16 && this.nombreP == 1))
							{
								...
								if (flagDir)
								{
									try
									{
										System.arraycopy(this.procesador.memDatos, dirVicFisica,
												this.cacheDatos[bloqueCach], 0, 4);
								...
								if (flagBus)
								{
									try
									{
										...
										if (flagDir)
										{
											try
											{
												System.arraycopy(this.procesador.p.memDatos, dirVicFisica,
														this.cacheDatos[bloqueCach], 0, 4);
								...
								case 2: //bloque esta en algun cache, pero a sido modificado
									...
									for (int i = 0; i < 3; i++)
									{
										if (pro.directorio[bloqueMem][i] == 1)
										{
											...
											if ((bloqueMem < 16 && this.nombreP == 0)
													|| (bloqueMem >= 16 && this.nombreP == 1))
											{
												System.arraycopy(n.cacheDatos[bloqueCach], 0, pro.memDatos, dirFisica,
														4);
											}
										}
									}
			...
			if (flagCache)
			{
				try
				{
					...
					if (bloqueVictMem == bloqueMem)
					{
						if (estado == 1)
						{
							Lock aux = (bloqueMem < 16) ? direcP0 : direcP1;
							boolean flagCacheRemota;
							Nucleo n;
							boolean dirflag = aux.tryLock();
							if (dirflag)
							{
								try
								{
									if (this.nombreP == 0 && bloqueMem < 16 || this.nombreP == 1 && bloqueMem >= 16)
									{
										esperarAvanceTic();
									} else
									{
						...
						this.cacheDatos[palabraMem][bloqueCach] = this.registro[reg];
						this.cacheDatos[4][bloqueCach] = 2;
						guardo = true;
					} else
					{
